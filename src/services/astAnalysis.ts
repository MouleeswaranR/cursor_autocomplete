...
//finding statement end using cursor position
export function findStatementEnd(
    ...
    function findSmallest(node: TreeSitter.Node): void {
        //if current node's position covers the range of cursor position
        if (containsPosition(node, cursorRow, cursorColumn)) {
            if (!bestNode || nodeSpan(node) <= nodeSpan(bestNode)) {//if nodespan becomes smaller, its going depther, example in nested if else blocks, in depth if conditions will go depther in their columns
                bestNode = node;
            }

            //looping over current node's child
    ...
    //find smallest indepth node with a current
    findSmallest(root);
    ...
    //going over parent nodes to find the statement start
    while (current.parent && current.parent !== root) {
        const parentType = current.parent.type;
        current = current.parent;
        
        //stopping for certain type of statements, cant go over
    ...
    //returning position of line statement end
    return {
